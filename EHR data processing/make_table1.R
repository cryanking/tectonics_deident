...
#  mutate(SurgDate=mdy(SurgDate)) %>%
  ...
  right_join( main_pop %>% unique  %>% mutate(Age=floor(as.numeric(difftime(Date, DoB, units="days"))/365.25) ) %>% select(CurrentMRN = MRN, SurgicalRecordID, SurgDate=Date ,OR_Abbrev , isTreatment, PtClass, Age, Sex), by=c('SurgicalRecordID' ) )  %>%
  ...
  left_join(join_mrns[, .( SurgicalRecordID, Service=SERVICE_collapsed)], by="SurgicalRecordID" ) -> table1_data  
