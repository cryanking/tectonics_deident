...
bp_only <- bp_only [  , .(low_map_time=sum(deltaT*(MAP< 60), na.rm=TRUE), total_map_time=max(Time, na.rm=TRUE)  , low_map_auc = sum(deltat2, na.rm=T) ) , by="orlogid"]
...
big_vent_time_up <- vector('list', length(all_ts))
