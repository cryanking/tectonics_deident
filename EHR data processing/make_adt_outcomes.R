...
epic_admits <- an_start_stop[epic_admits, on="CurrentMRN", allow.cartesian=T ]
...
epic_admits  [ which((prev_status == "Discharge") & (ADT_EVENT %chin% c("Admission", "Transfer In"))) -1 , ADT_EVENT:="Transfer Out" ]
epic_admits  [(prev_status == "Discharge") & (ADT_EVENT== "Admission") , ADT_EVENT:="Transfer In" ]
...
clarity_death <- unique(an_start_stop[,.(orlogid, CurrentMRN)])[clarity_death, on="CurrentMRN" , allow.cartesian=TRUE]
clarity_death[, CurrentMRN:=NULL] 
...
preop_los <- epic_admits[(ADT_EVENT=="Admission") & (EFFECTIVE_TIME < AnestStop), .(admit_y =   max(EFFECTIVE_TIME) ), by="orlogid" ]
discharge_time <- epic_admits [(EFFECTIVE_TIME > AnestStop) & (ADT_EVENT=="Discharge") , .( discharge_time=min(EFFECTIVE_TIME) ) , by="orlogid"]

discharge_time <- clarity_death[discharge_time, on="orlogid" ]
discharge_time[, discharge_time:= pmin(discharge_time, Death_Date, na.rm=T) ] 
...
adt_data <- merge(merge(merge(merge(first_post_admit , discharge_time , all=T,  by="orlogid"), an_start_stop, all=T,  by="orlogid"), preop_ICU , all=TRUE,  by="orlogid"), preop_los,  by="orlogid", all=T)
...
postop_ICU <-  copy(epic_admits [(EFFECTIVE_TIME > AnestStop -dhours(2) ) ][grepl(DEPARTMENT_NAME, pattern="BJH\\s\\d")] )
setorder(postop_ICU, orlogid, EFFECTIVE_TIME)

## is the first unit an icu, regardless of how far in the future it is
postop_ICU_late <- postop_ICU[ postop_ICU[, .I[1], by="orlogid" ]$V1 ]
postop_ICU_late <- postop_ICU_late[ , .(ICU=grepl(DEPARTMENT_NAME, pattern="ICU") , unit=DEPARTMENT_NAME, orlogid , admit_day=as.numeric(difftime(EFFECTIVE_TIME,AnestStop, units="days"  )) ) ]
...
epic_admits <- an_records[admits, on="CurrentMRN", allow.cartesian=T ]
...
adt_data <- merge(merge(merge(merge(first_post_admit , discharge_time , all=T,  by="orlogid"), an_records, all=T,  by="orlogid"), preop_ICU , all=TRUE,  by="orlogid"), preop_los,  by="orlogid", all=T)
...
## during this stay
postop_ICU <- discharge_time[postop_ICU , on="orlogid", nomatch=NA]
postop_ICU[is.na(discharge_time) , discharge_time :=last_date ] 
postop_ICU <- postop_ICU[ EFFECTIVE_TIME <= discharge_time]
...
epic_admits <- join_mrns[, .(AnestStart=AN_START_DATETIME, orlogid=SurgicalRecordID,AnestStop= AN_STOP_DATETIME, CurrentMRN=MRN )][admits, on="CurrentMRN", allow.cartesian=T ]
...
adt_data <- merge(merge(merge(merge(first_post_admit , discharge_time , all=T,  by="orlogid"), join_mrns[, .(AnestStart=AN_START_DATETIME, orlogid=SurgicalRecordID,AnestStop= AN_STOP_DATETIME, CurrentMRN=MRN )], all.x=T,  by="orlogid"), preop_ICU , all=TRUE,  by="orlogid"), preop_los,  by="orlogid", all=T)
...
adt_outcomes_merged <- fread("/research/intermediates/adt_imports.csv")
}
