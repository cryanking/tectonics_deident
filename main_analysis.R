...
main_analysis <- function(local_data, primary_outcomes = c('CAM', 'RespF' ,'AKI' , 'EarlyDeath'), do_correct=TRUE ) {
  ...
  primarytab$p <- ""
  primarytab$coef2 <- ""
  primarytab$missing_rate <- ""
  ntest2 <- length(primary_outcomes)

  for( outcome in  primary_outcomes ) {
    this_data <-  cbind(local_data[ , .SD[[outcome]]] , local_data[ , .(x=as.numeric(isTreatment),cluster_id )])
    colnames(this_data) <- c("y", "x", "z")
    primarytab[primarytab$outcome ==outcome, "missing_rate"] <- this_data[["y"]] %>% is.na %>% mean %>% multiply_by(100) %>% round(1) %>% as.character
    this_data <- this_data[is.finite(y)]
    this_data <- this_data[is.finite(x)]
...
if(file.exists("/research/outputs/reidentify_data.csv") ) {
  ...
  processed_data_set <- reident[, .(orlogid_encoded, thisq= quarter(SurgDate, with_year=T) )][processed_data_set, on="orlogid_encoded"]
...
fix_table_format <- . %>% 
  mutate(outcome = outcome %>% case_match("CAM"~"Delirium", "RespF"~"Respiratory Failure", "AKI"~"AKI", "EarlyDeath"~"30-day mortality", "sumOuts"~"Composite", "Abx_success"~"Antibiotic redosing", "temp_good"~"Normothermia","hypotension_fraction"~"Fraction time hypotensive" , "pip_good_frac"~"Fraction time low PIP", "first_glucose"~ "Hyperglycemia" , "no_gaps"~"Anesthetic delivery", "eff_flow"~"Fraction time efficient gas flow", "ICU"~"ICU utilization", "postop_los"~"Length of stay", "ICULoS"~"ICU length of stay", "mean_temp"~"Mean temperature" , "low_map_auc"~"Time weighted hypotension", "ratio_cre"~"Change in creatinine" , "pip_mean"~"Mean PIP", "CAM2"~"Delirium (missing as false)" ,  .default=outcome) ) %>%
  mutate(statement = paste0("[ ACT ", interv %>% sub(pattern=")", replacement="%)", fixed=T), " vs control ", control %>% sub(pattern=")", replacement="%)", fixed=T), ", RR ", coef    %>% sub(pattern="(", replacement="(95% CI ", fixed=T ) %>% sub(pattern=",", replacement=" to"), " p=", p_corrected  , "]") ) %>%
  select(Outcome=outcome, Intervention=interv, Control=control, `GEE coef (95% CI)`=coef, `p value`=p_corrected, `Missing rate`=missing_rate,  `GEE coef (99.5% CI)` = coef2, `uncorrected p` = p,  everything()  ) %>% mutate_all(as.character)

fix_table_format_exp <- . %>% 
  mutate(outcome = outcome %>% case_match("CAM"~"Delirium", "RespF"~"Respiratory Failure", "AKI"~"AKI", "EarlyDeath"~"30-day mortality", "sumOuts"~"Composite", "Abx_success"~"Antibiotic redosing", "temp_good"~"Normothermia","hypotension_fraction"~"Fraction time hypotensive" , "pip_good_frac"~"Fraction time low PIP", "first_glucose"~ "Hyperglycemia" , "no_gaps"~"Anesthetic delivery", "eff_flow"~"Fraction time efficient gas flow", "ICU"~"ICU utilization", "postop_los"~"Length of stay", "ICULoS"~"ICU length of stay", "mean_temp"~"Mean temperature" , "low_map_auc"~"Time weighted hypotension", "ratio_cre"~"Change in creatinine" , "pip_mean"~"Mean PIP", "CAM2"~"Delirium (missing as false)" ,  .default=outcome) ) %>% 
  select(Outcome=outcome,  Intervention=interv, Control=control, `GEE coef`=coef, `p value`=p, `Missing rate`=missing_rate,  `GEE coef (99.5% CI)` = coef2, `uncorrected p` = p,  everything() ) %>% mutate_all(as.character)
...
alt_spec_Variables <- c('ICULoS'  , 'mean_temp' ,'low_map_auc' , 'ratio_cre' , 'pip_mean' ,  "CAM2" , "first_glucose_restr")
